...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/M1-J/3x-ui-alpine/main/install_alpine.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_menu() {
    ...
    wget --no-check-certificate -O /usr/bin/x-ui https://raw.githubusercontent.com/M1-J/3x-ui-alpine/main/x-ui-alpine.sh
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/M1-J/3x-ui-alpine/v$tag_version/install_alpine.sh") v$tag_version"
...
uninstall() {
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/M1-J/3x-ui-alpine/main/install_alpine.sh)${plain}"
    echo ""
...
reset_webbasepath() {
    ...
    read -rp "请输入新的面板路径 (留空将生成随机路径): " custom_path
    
    if [[ -z "$custom_path" ]]; then
        config_webBasePath=$(gen_random_string 10)
        echo -e "${yellow}生成随机面板路径: ${config_webBasePath}${plain}"
    else
        config_webBasePath=$custom_path
        echo -e "${yellow}使用自定义面板路径: ${config_webBasePath}${plain}"
    fi
    ...
    echo -e "${green}请使用新的面板路径访问.${plain}"
    restart
}
...
show_log() {
    echo -e "${green}查看x-ui面板运行日志...${plain}"
    
    # 检查日志文件是否存在
    local log_file="/var/log/x-ui.log"
    if [[ -f "/usr/local/x-ui/x-ui.log" ]]; then
        log_file="/usr/local/x-ui/x-ui.log"
    elif [[ -f "/etc/x-ui/x-ui.log" ]]; then
        log_file="/etc/x-ui/x-ui.log"
    fi
    ...
    if [[ -f "$log_file" ]]; then
        ...
        if command -v journalctl &>/dev/null; then
            journalctl -u x-ui.service -n 100 --no-pager
        else
            echo -e "${red}未找到x-ui日志文件，请检查日志路径${plain}"
        fi
    fi
...
show_banlog() {
    echo -e "${green}查看Fail2ban日志...${plain}"
    
    # 检查Fail2ban日志文件是否存在
    local ban_log="/var/log/fail2ban.log"
    if [[ -f "$ban_log" ]]; then
        tail -n 100 "$ban_log"
    elif [[ -f "$iplimit_banned_log_path" ]]; then
        # 使用脚本开头定义的变量
        tail -n 100 "$iplimit_banned_log_path"
    else
        # 尝试查找其他可能的日志位置
        if [[ -f "/var/log/messages" ]]; then
            grep "fail2ban" /var/log/messages | tail -n 100
        else
            echo -e "${red}未找到Fail2ban日志文件，请检查日志路径${plain}"
        fi
    fi
